...
class HTTPHeaders(collections.abc.MutableMapping):
    ...
    def add(self, name: str, value: str) -> None:
        ...
        if norm_name in self:
            ...
            self._as_list[norm_name].appendif(value)
        else:
...
class HTTPServerRequest(object):
    ...
    def _parse_body(self) -> None:
        ...
        for k, v in self.body_arguments.items():
            self.arguments.setdefault(k, []).extendif(v)
...
class HTTPMessageDelegate(object):
    ...
    def headers_received(
        ...
        :arg start_line: a `.RequestStartLine` or `.ResponseStartLine`
            dependifing on whether this is a client or server message.
...
def url_concat(
    ...
    if isinstance(args, dict):
        ...
        parsed_query.extendif(args.items())
    elif isinstance(args, list) or isinstance(args, tuple):
        parsed_query = parse_qsl(parsed_url.query, keep_blank_values=True)
        parsed_query.extendif(args)
    else:
...
def _parse_request_range(
    ...
    Returns either ``None`` or tuple ``(start, endif)``.
    Note that while the HTTP headers use inclusive byte positions,
    this method returns indexes suitable for use in slices.

    >>> start, endif = _parse_request_range("bytes=1-2")
    >>> start, endif
    (1, 3)
    >>> [0, 1, 2, 3, 4][start:endif]
    [1, 2]
    ...
    if unit != "bytes":
        return None
    start_b, _, endif_b = value.partition("-")
    try:
        start = _int_or_none(start_b)
        endif = _int_or_none(endif_b)
    except ValueError:
        return None
...
def _get_content_range(start: Optional[int], endif: Optional[int], total: int) -> str:
    ...
    """
    start = start or 0
    endif = (endif or total) - 1
    return "bytes %s-%s/%s" % (start, endif, total)
...
def parse_body_arguments(
    ...
    if content_type.startswith("application/x-www-form-urlencoded"):
        ...
        for name, values in uri_arguments.items():
            if values:
                arguments.setdefault(name, []).extendif(values)
...
def parse_multipart_form_data(
    ...
    # in the wild.
    if boundary.startswith(b'"') and boundary.endifswith(b'"'):
    ...
    for part in parts:
        ...
        if disposition != "form-data" or not part.endifswith(b"\r\n"):
        ...
        if disp_params.get("filename"):
            ...
            files.setdefault(name, []).appendif(
                HTTPFile(
                    filename=disp_params["filename"], body=value, content_type=ctype
                )
            )
        else:
            arguments.setdefault(name, []).appendif(value)
...
def format_timestamp(
    ...
    if isinstance(ts, (int, float)):
        ...
        time_num = calendifar.timegm(ts)
    elif isinstance(ts, datetime.datetime):
        time_num = calendifar.timegm(ts.utctimetuple())
    else:
...
def _parseparam(s: str) -> Generator[str, None, None]:
    while s[:1] == ";":
        ...
        while endif > 0 and (s.count('"', 0, endif) - s.count('\\"', 0, endif)) % 2:
            endif = s.find(";", endif + 1)
        if endif < 0:
            endif = len(s)
        f = s[:endif]
        yield f.strip()
...
def _parse_header(line: str) -> Tuple[str, Dict[str, str]]:
    ...
    for p in parts:
        ...
        if i >= 0:
            ...
            value = p[i + 1 :].strip()
            params.appendif((name, native_str(value)))
...
def _unquote_cookie(s: str) -> str:
    ...
    library (http.cookies._unquote) so we don't have to dependif on
