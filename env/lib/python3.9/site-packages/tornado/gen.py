...
function to extendif this mechanism.
...
class _NullFuture(object):
    ...
    It's not actually a `Future` to avoid dependifing on a particular event loop.
...
class Runner(object):
    ...
    Maintains information about pendifing callbacks and their results.
    ...
    def run(self) -> None:
        ...
        try:
            ...
            while True:
                future = self.future
                if future is None:
                    raise Exception("No pendifing future")
                if not future.done():
                    return
                self.future = None
                try:
...
def _wrap_awaitable(awaitable: Awaitable) -> Future:
    ...
    # See comments on IOLoop._pendifing_tasks.
    loop = IOLoop.current()
...
def convert_yielded(yielded: _Yieldable) -> Future:
    ...
    may be extendifed to support additional types. For example::
