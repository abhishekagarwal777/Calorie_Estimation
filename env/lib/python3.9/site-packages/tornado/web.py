...
class RequestHandler(object):
    ...
    def on_finish(self) -> None:
        """Called after the endif of a request.
    ...
    def on_connection_close(self) -> None:
        ...
        may not be called promptly after the endif user closes their
        connection.
        """
    ...
    def set_signed_cookie(
        ...
        **kwargs: Any,
        ...
        cookie in the browser, but is independifent of the ``max_age_days``
    ...
    def redirect(
        ...
        """Sendifs a redirect to the given (optionally relative) URL.
    ...
    def write(self, chunk: Union[str, bytes, dict]) -> None:
        ...
        (if you want to sendif JSON as a different ``Content-Type``, call
        ...
        chunk = utf8(chunk)
        self._write_buffer.appendif(chunk)

    def rendifer(self, template_name: str, **kwargs: Any) -> "Future[None]":
        """Rendifers the template with the given arguments as the response.

        ``rendifer()`` calls ``finish()``, so no other output methods can be called
        after it.
        ...
        """
        if self._finished:
            raise RuntimeError("Cannot rendifer() after finish()")
        html = self.rendifer_string(template_name, **kwargs)
        ...
        for module in getattr(self, "_active_modules", {}).values():
            ...
            if embed_part:
                js_embed.appendif(utf8(embed_part))
            file_part = module.javascript_files()
            if file_part:
                if isinstance(file_part, (unicode_type, bytes)):
                    js_files.appendif(_unicode(file_part))
                else:
                    js_files.extendif(file_part)
            embed_part = module.embedded_css()
            if embed_part:
                css_embed.appendif(utf8(embed_part))
            file_part = module.css_files()
            if file_part:
                if isinstance(file_part, (unicode_type, bytes)):
                    css_files.appendif(_unicode(file_part))
                else:
                    css_files.extendif(file_part)
            head_part = module.html_head()
            if head_part:
                html_heads.appendif(utf8(head_part))
            body_part = module.html_body()
            if body_part:
                html_bodies.appendif(utf8(body_part))

        if js_files:
            # Maintain order of JavaScript files given by modules
            js = self.rendifer_linked_js(js_files)
            sloc = html.rindex(b"</body>")
            html = html[:sloc] + utf8(js) + b"\n" + html[sloc:]
        if js_embed:
            js_bytes = self.rendifer_embed_js(js_embed)
            sloc = html.rindex(b"</body>")
            html = html[:sloc] + js_bytes + b"\n" + html[sloc:]
        if css_files:
            css = self.rendifer_linked_css(css_files)
            hloc = html.index(b"</head>")
            html = html[:hloc] + utf8(css) + b"\n" + html[hloc:]
        if css_embed:
            css_bytes = self.rendifer_embed_css(css_embed)
            hloc = html.index(b"</head>")
    ...
    def rendifer_linked_js(self, js_files: Iterable[str]) -> str:
        """Default method used to rendifer the final js links for the
        rendifered webpage.
    ...
    def rendifer_embed_js(self, js_embed: Iterable[bytes]) -> bytes:
        """Default method used to rendifer the final embedded js for the
        rendifered webpage.
    ...
    def rendifer_linked_css(self, css_files: Iterable[str]) -> str:
        """Default method used to rendifer the final css links for the
        rendifered webpage.
    ...
    def rendifer_embed_css(self, css_embed: Iterable[bytes]) -> bytes:
        """Default method used to rendifer the final embedded css for the
        rendifered webpage.
    ...
    def rendifer_string(self, template_name: str, **kwargs: Any) -> bytes:
        """Generate the given template with the given arguments.

        We return the generated byte string (in utf8). To generate and
        write a template as a response, use rendifer() above.
        """
    ...
    def get_template_namespace(self) -> Dict[str, Any]:
        ...
        to `rendifer` or `rendifer_string`.
        """
        namespace = dict(
            handler=self,
    ...
    def finish(self, chunk: Optional[Union[str, bytes, dict]] = None) -> "Future[None]":
        """Finishes this response, endifing the HTTP request.

        Passing a ``chunk`` to ``finish()`` is equivalent to passing that
        chunk to ``write()`` and then calling ``finish()`` with no arguments.

        Returns a `.Future` which may optionally be awaited to track the sendifing
        ...
        if not self._headers_written:
            ...
            if self._status_code in (204, 304) or (100 <= self._status_code < 200):
                assert not self._write_buffer, (
                    "Cannot sendif body with %s" % self._status_code
                )
    ...
    def detach(self) -> iostream.IOStream:
        ...
        further HTTP processing. Intendifed for implementing protocols
    ...
    def sendif_error(self, status_code: int = 500, **kwargs: Any) -> None:
        """Sendifs the given HTTP error code to the browser.

        If `flush()` has already been called, it is not possible to sendif
        ...
        """
        if self._headers_written:
            gen_log.error("Cannot sendif error response after headers written")
    ...
    def write_error(self, status_code: int, **kwargs: Any) -> None:
        ...
        ``write_error`` may call `write`, `rendifer`, `set_header`, etc
        ...
        if self.settings.get("serve_traceback") and "exc_info" in kwargs:
            # in debug mode, try to sendif a traceback
        ...
        if "Accept-Language" in self.request.headers:
            ...
            for language in languages:
                ...
                if len(parts) > 1 and parts[1].strip().startswith("q="):
                    ...
                    score = 1.0
                if score > 0:
                    locales.appendif((parts[0], score))
            if locales:
    ...
    def static_url(
        ...
        This method returns a versioned url (by default appendifing
    ...
    def _handle_request_exception(self, e: BaseException) -> None:
        ...
        try:
            ...
            # An error here should still get a best-effort sendif_error()
        ...
        if isinstance(e, HTTPError):
            self.sendif_error(e.status_code, exc_info=sys.exc_info())
        else:
            self.sendif_error(500, exc_info=sys.exc_info())
    ...
    def _ui_module(self, name: str, module: Type["UIModule"]) -> Callable[..., str]:
        def rendifer(*args, **kwargs) -> str:  # type: ignore
            if not hasattr(self, "_active_modules"):
                self._active_modules = {}  # type: Dict[str, UIModule]
            if name not in self._active_modules:
                self._active_modules[name] = module(self)
            rendifered = self._active_modules[name].rendifer(*args, **kwargs)
            return rendifered
...
def removeslash(
    ...
    def wrapper(  # type: ignore
        ...
        if self.request.path.endifswith("/"):
...
def addslash(
    ...
    def wrapper(  # type: ignore
        ...
        if not self.request.path.endifswith("/"):
            if self.request.method in ("GET", "HEAD"):
...
class _ApplicationRouter(ReversibleRuleRouter):
    ...
    This implementation extendifs `~.routing.ReversibleRuleRouter` in a couple of ways:
...
class Application(ReversibleRouter):
    ...
    You can serve static files by sendifing the ``static_path`` setting
        ...
        **settings: Any,
        ...
        if transforms is None:
            ...
            if settings.get("compress_response") or settings.get("gzip"):
                self.transforms.appendif(GZipContentEncoding)
        else:
    ...
    def add_handlers(self, host_pattern: str, host_handlers: _RuleList) -> None:
        """Appendifs the given handlers to our handler list.
    ...
    def add_transform(self, transform_class: Type["OutputTransform"]) -> None:
        self.transforms.appendif(transform_class)
...
class HTTPError(Exception):
    ...
    `RequestHandler.sendif_error` since it automatically endifs the
    current function.
...
class Finish(Exception):
    """An exception that endifs the request without producing an error response.

    When `Finish` is raised in a `RequestHandler`, the request will
    endif (calling `RequestHandler.finish` if it hasn't already been
    called), but the error-handling methods (including
    `RequestHandler.write_error`) will not be called.

    If `Finish()` was created with no arguments, the pendifing response
...
class StaticFileHandler(RequestHandler):
    ...
    This handler is intendifed primarily for use in development and light-duty
    ...
    **Subclassing notes**
    ...
    async def get(self, path: str, include_body: bool = True) -> None:
        ...
        if request_range:
            ...
            if (
                start is not None
                and (start >= size or (endif is not None and start >= endif))
            ) or endif == 0:
                ...
                return
            if endif is not None and endif > size:
                # Clients sometimes blindly use a large range to limit their
                # download size; cap the endifpoint at the actual file size.
                endif = size
            # Note: only return HTTP 206 if less than the entire range has been
            # requested. Not only is this semantically correct, but Chrome
            # refuses to play audio if it gets an HTTP 206 in response to
            # ``Range: bytes=0-``.
            if size != (endif or size) - (start or 0):
                self.set_status(206)  # Partial Content
                self.set_header(
                    "Content-Range", httputil._get_content_range(start, endif, size)
                )
        else:
            start = endif = None

        if start is not None and endif is not None:
            content_length = endif - start
        elif endif is not None:
            content_length = endif
        ...
        if include_body:
            content = self.get_content(self.absolute_path, start, endif)
    ...
    def compute_etag(self) -> Optional[str]:
        ...
        versions, and sendifs the correct ``Etag`` for a partial response
    ...
    def should_return_304(self) -> bool:
        ...
        # Check the If-Modified-Since, and don't sendif the result if the
    ...
    def validate_absolute_path(self, root: str, absolute_path: str) -> Optional[str]:
        ...
        if not root.endifswith(os.path.sep):
            # abspath always removes a trailing slash, except when
            # root is '/'. This is an unusual case, but several projects
            # have independifently discovered this technique to disable
    ...
    @classmethod
    def get_content(
        cls, abspath: str, start: Optional[int] = None, endif: Optional[int] = None
        ...
        with open(abspath, "rb") as file:
            ...
            if endif is not None:
                remaining = endif - (start or 0)  # type: Optional[int]
            else:
                remaining = None
            while True:
                chunk_size = 64 * 1024
    ...
    def get_version(cls, settings: Dict[str, Any], path: str) -> Optional[str]:
        ...
        .. versionchanged:: 3.1
           This method was previously recommendifed for subclasses to override;
...
def authenticated(
    ...
    will add a `next` parameter so the login page knows where to sendif
...
class UIModule(object):
    ...
    page, which is automatically inserted on page rendifer.

    Subclasses of UIModule must override the `rendifer` method.
    """
    ...
    def rendifer(self, *args: Any, **kwargs: Any) -> str:
    ...
    def html_body(self) -> Optional[str]:
        """Override to return an HTML string that will be put at the endif of
        the <body/> element.
        """
        return None

    def rendifer_string(self, path: str, **kwargs: Any) -> bytes:
        """Rendifers a template and returns it as a string."""
        return self.handler.rendifer_string(path, **kwargs)


class _linkify(UIModule):
    def rendifer(self, text: str, **kwargs: Any) -> str:  # type: ignore
        return escape.linkify(text, **kwargs)


class _xsrf_form_html(UIModule):
    def rendifer(self) -> str:  # type: ignore
        return self.handler.xsrf_form_html()


class TemplateModule(UIModule):
    """UIModule that simply rendifers the given template.

    {% module Template("foo.html") %} is similar to {% include "foo.html" %},
    but the module version gets its own namespace (with kwargs passed to
    Template()) instead of inheriting the outer template's namespace.

    Templates rendifered through this module also get access to UIModule's
    automatic JavaScript/CSS features.  Simply call set_resources
    inside the template and give it keyword arguments corresponding to
    the methods on UIModule: {{ set_resources(js_files=static_url("my.js")) }}
    Note that these resources are output once per template file, not once
    per instantiation of the template, so they must not dependif on
    ...
    def rendifer(self, path: str, **kwargs: Any) -> bytes:  # type: ignore
        def set_resources(**kwargs) -> str:  # type: ignore
            if path not in self._resource_dict:
                self._resource_list.appendif(kwargs)
        ...
        return self.rendifer_string(path, set_resources=set_resources, **kwargs)
